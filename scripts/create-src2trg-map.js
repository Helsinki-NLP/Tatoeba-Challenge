...
function onEachFeature(feature, layer) {
    ...
    if (geojson.properties.legend.hasOwnProperty(trgid)) {
        if (trglangs.hasOwnProperty(trgid)) {
            trglangs[trgid].push(layer);
        } else {
            trglangs[trgid] = [];
	    trglangs[trgid].push(layer);
        }
...
for (var id in trglangs) {
    if (trglangs.hasOwnProperty(id)) {
        flayers[geojson.properties.legend[id]] = L.layerGroup(trglangs[id]);
